/**
 * notifications.js — UpcycleConnect Espace Pro
 * Module Notifications
 *
 * Responsabilités :
 *   1. Lister toutes les notifications de l'utilisateur Pro (paginées)
 *   2. Permettre le marquage lu (unitaire + tout marquer)
 *   3. Permettre la suppression d'une notification
 *   4. Filtrer par type (toutes, non lues, urgentes, infos)
 *
 * Pattern : s'enregistre dans window.ProModules.notifications
...
(function () {
  ...
  let _state = {
    notifications: [],
    activeFilter: "all", // 'all' | 'unread' | 'urgente' | 'info'
    session: null,
  };
  ...
  /**
   * Tente d'appeler l'API Go, repli sur mock si indisponible.
   */
  ...
  /** Couleur et libellé selon le type de notification */
  ...
  function renderList(notifications, filter) {
    ...
    // Compteurs dans les onglets de filtre
  ...
  /**
   * Filtre les notifications sans re-fetch.
   * Appelé par les boutons de filtre dans la vue.
   */
  ...
  /**
   * Marque une notification comme lue (optimistic update + API).
  ...
  async function markRead(notifId) {
    ...
    // Synchronise le compteur global dans le Store
  ...
  /**
   * Supprime une notification (optimistic update + API).
   * @param {string} notifId
   */
  ...
  async function loadData() {
    ...
    // Met à jour le compteur global dans le Store
