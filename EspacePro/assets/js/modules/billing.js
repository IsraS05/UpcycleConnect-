/**
 * billing.js — UpcycleConnect Espace Pro
 * Module B — Tasks 43-44 : Factures + génération/téléchargement PDF
 *
 * Responsabilités :
 *   Task 43 : Liste paginée des factures avec filtres (statut, année)
 *   Task 44 : Aperçu HTML de la facture + génération PDF (window.print() ou blob API)
 *
 * Stratégie PDF :
 *   1. Si l'API Go retourne un blob PDF → téléchargement direct
 *   2. Sinon → génération côté client via window.print() avec CSS @media print
 *   Les deux voies coexistent et se complètent.
 */
...
(function () {
  ...
  let _state = {
    ...
    currentYear: "all",
    currentPage: 1,
    previewInvoice: null, // Facture actuellement affichée dans le modal
  };
  ...
  /**
   * Téléchargement PDF — stratégie duale :
   *   1. Essaie de récupérer le PDF depuis l'API Go (blob)
   *   2. Si indisponible → génère via window.print() avec CSS @media print
   */
  async function downloadPdf(invoiceId) {
    ...
    try {
      ...
      if (!error && blob && blob.size > 0) {
        ...
        // Fallback : impression navigateur avec CSS @media print
        _printInvoice(id);
      }
    } catch {
      _printInvoice(id);
   ...
   * On ouvre une nouvelle fenêtre avec le HTML de la facture + CSS print.
   */
  ...
  // ACTIONS PUBLIQUES — Filtres & pagination
  ...
  function goToPage(page) {
    ...
    renderTable();
    // Scroll top du tableau
    document
      .getElementById("invoices-table")
      ?.scrollIntoView({ behavior: "smooth", block: "start" });
  }

  /** Export CSV de toutes les factures filtrées */
  function exportAll() {
    const rows = [
      [
        "N° Facture",
