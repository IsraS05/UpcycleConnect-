/**
 * subscription.js — UpcycleConnect Espace Pro
 * Module B — Task 42 : Gestion contrat + changement de plan
 *
 * Responsabilités :
 *   - Afficher le contrat actuel (plan, dates, statut)
 *   - Afficher le comparatif des 3 plans disponibles
 *   - Gérer l'upgrade/downgrade avec confirmation modale
 *   - Gérer l'annulation avec confirmation par saisie texte
 */
...
(function () {
  ...
  // ─────────────────────────────────────────
  // CATALOGUE DES PLANS
  // Source de vérité pour l'affichage — à synchroniser avec la BDD
  // ─────────────────────────────────────────
  ...
  let _state = {
    subscription: null,
    pendingPlanId: null, // Plan sélectionné en attente de confirmation
  };
  ...
  async function fetchOrMock(apiFn, mockData, label) {
    try {
      ...
      // Normaliser les sql.NullString Go → string simple
  ...
  function confirmCancel() {
    ...
    // Activer le bouton seulement si "ANNULER" est saisi
  ...
  async function executeCancellation() {
    ...
    try {
      ...
      // En mode mock, on simule quand même
