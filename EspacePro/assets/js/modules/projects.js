/**
 * projects.js — UpcycleConnect Espace Pro
 * Module D — Tasks 52-55
 *
 * Responsabilités :
 *   Task 52 : Liste projets filtres statut, tri, pagination, avancement rapide d'étape
 *   Task 53 : Éditeur multi-étapes (4 étapes : Infos → Matériaux → Timeline → Récap)
 *   Task 54 : Mise à jour d'étape (COLLECTE → TRANSFORMATION → VENTE → TERMINE)
 *   Task 55 : Algorithme calcul impact carbone (ADEME Base Carbone v22)
 *
 * Pattern :
 *   ProModules.projects      → gestion de la liste
 *   ProModules.projectEditor → éditeur multi-étapes
 *   Les deux partagent les mêmes helpers et données mock
 */
...
(function () {
  ...
  // ─────────────────────────────────────────
  // ALGO IMPACT CARBONE (Task 55)
  // Formule : kg_co2 = poids_kg × coeff_matiere
  // Source : ADEME Base Carbone v22
  // ─────────────────────────────────────────
  ...
  async function advanceStep(projectId) {
    const id = parseInt(projectId) || projectId; // convertit "2" → 2
    ...
    try {
      await API.projects.updateStep(id, next); // ← utiliser id (entier)
    } catch {
      /* simulation */
    }
  ...
  // ═══════════════════════════════════════════
  // PARTIE 2 : ÉDITEUR MULTI-ÉTAPES (Task 53)
  // ═══════════════════════════════════════════
  ...
  async function nextStep() {
    ...
    if (_ed.currentStep === 0) {
      ...
      // Supprimer is-invalid si tout va bien
      const priceInput = document.getElementById("ed-price");
