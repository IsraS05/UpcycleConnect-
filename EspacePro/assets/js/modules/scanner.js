/**
 * scanner.js — UpcycleConnect Espace Pro
 * Module C — Tasks 50-51
 *
 * Responsabilités :
 *   Task 50 : Accès caméra via getUserMedia + sélection caméra avant/arrière + torche
 *   Task 51 : Décodage QR Code (jsQR) en temps réel + validation API Go + confirmation collecte
 *
 * Librairie QR : jsQR (CDN, ~24kb gzip)
 *   Format attendu : UC-C<id_conteneur>-OBJ-<id_objet>
 *   Ex : UC-C047-OBJ-0012
 *
 * Stratégie :
 *   - Canvas caché reçoit les frames video → jsQR décode
 *   - Debounce sur la détection (évite le double-déclenchement)
 *   - Validation API Go → si non disponible, simulation locale
 *   - Historique conservé dans sessionStorage (perdu à la fermeture)
 */
...
(function () {
  ...
  // Regex format QR attendu
  const QR_REGEX = /^UC-C(\w+)-OBJ-(\w+)$/i;

  // Cooldown entre deux détections (ms) — évite le double-scan
  const SCAN_COOLDOWN_MS = 2500;

  // ─────────────────────────────────────────
  // DONNÉES SIMULÉES (objets dans les conteneurs)
  // ─────────────────────────────────────────
  ...
  let _state = {
    stream: null, // MediaStream actif
    scanning: false, // Boucle de scan active
    animFrameId: null, // requestAnimationFrame handle
    facingMode: "environment", // 'environment' | 'user'
    torchOn: false,
    torchTrack: null, // Track vidéo (pour torche)
    lastScanTime: 0, // Timestamp dernier scan réussi
    pendingResult: null, // Résultat en attente de confirmation
    history: [], // Historique scans (sessionStorage)
    canvas: null, // Canvas caché pour jsQR
    ctx: null,
  };
  ...
  // ─────────────────────────────────────────
  // BOUCLE DE SCAN (jsQR + requestAnimationFrame)
  // ─────────────────────────────────────────
  ...
  function startScanLoop() {
    ...
    function tick() {
      ...
      // Dessiner la frame courante dans le canvas
      ...
      if (code?.data) {
        const now = Date.now();
        // Cooldown : évite le double-déclenchement sur le même QR
  ...
  async function onQRDetected(rawCode) {
    ...
    // Afficher le feedback visuel de succès
  ...
  function showScanSuccess(rawCode) {
    ...
    if (overlay) {
      ...
      // Masquer après 2s (puis traitement)
      setTimeout(() => {
  ...
  async function validateCode(code) {
    try {
      ...
      if (error || !data) {
        // Fallback sur la base mock locale
  ...
  async function processResult(result) {
    ...
    // Succès — afficher l'objet et le conteneur trouvés
  ...
  async function confirmCollection() {
    ...
    // Mettre à jour le store (score, objets)
  ...
  // ─────────────────────────────────────────
  // ACTIONS PUBLIQUES — Toggle / Caméra
  // ─────────────────────────────────────────
  ...
  async function init(ctx) {
    ...
    // Pré-charger jsQR en arrière-plan (pas bloquant)
    loadJsQR().catch(() => {});
  }
